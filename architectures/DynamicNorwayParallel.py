...
# Parallel configuration of DynamicNorway
...
class DynamicNorway(nn.Module):
    def __init__(self, b=2, m=1.5, num_classes=8):
        ...
        total_input_channels = 0  # To accumulate the total input channels for the decoder
        ...
        for i in range(1, b + 1):  # Repeat b times
            ...
            self.blocks.append(BlockWithSkipConnection(64, out_channels))
            
            # Update the total input channels for the decoder
        ...
        total_input_channels += 48
        out_channels = int(64 * (m ** b))  # C_b = 64 * m^b
    
        # Add a the separable block to the decoder
        self.decoder_block = BlockWithSkipConnection(total_input_channels, out_channels)
    ...
    def forward(self, x):
        ...
        outputs = []  # List to store outputs from each block
        for block in self.blocks:
            block_out = block(x)  # Apply the separable blocks and skip connection in parallel
            outputs.append(block_out)  # Store the output

        x = torch.cat(outputs + [identity], dim=1)  # Concatenate along channels
